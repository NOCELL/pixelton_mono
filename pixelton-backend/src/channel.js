...
const init = async () => {
  ...
  const AMOUNT_CHECKOUT = toNano("1");
  const NETWORK_FEE = toNano("0.5");
  ...
  const channelInitState = {
    balanceA: AMOUNT_CHECKOUT, // A's initial balance in Toncoins. Next A will need to make a top-up for this amount
  ...
  const channelConfig = {
    channelId: new BN("5"), // Channel ID, for each new channel there must be a new ID
  ...
  await fromWalletA
    ...
    .send(channelInitState.balanceA.add(NETWORK_FEE)); // +0.05 TON to network fees

  await fromWalletB
    .topUp({ coinsA: toNano("0"), coinsB: toNano("0.0001") })
    .send(channelInitState.balanceB.add(NETWORK_FEE)); // +0.05 TON to network fees
  ...
  await fromWalletA.init(channelInitState).send(NETWORK_FEE);
  ...
  const resultClose = await fromWalletA
    .close({
      ...channelState3,
